🔗 AI対応設定ファイル: [.ai-config.json](.ai-config.json)

### 📌 プロジェクトルートの定義

このプロジェクトでは `.project-root` ファイルを使用して、明示的にルートディレクトリを指定しています。

- `.project-root` は空ファイルでも構いません。
- ツールやスクリプトが「プロジェクトの起点」を判定するための目印として利用します。
- ZIPなどで受け渡す際にもこのファイルを含めておくことで、自動解析やパス解決が安定します。

📁 例：
```
kadai_01-feature-kd01-20250728/
└── kadai_01-feature-kd01-20250728/
...
### 🆕 最近の変更点（2025/07/28）
- ボタン／フォームのUI仕様を統一（サイズ、色、hover時の動作など）
- 成功時メッセージの表示内容・UXを改善（持続時間・表示位置の調整）
- Tailwindユーティリティクラスの整理とCSSクリーンアップ
- UIガイドラインのドキュメントを整備し、共通ルールを明文化
- 一般ユーザー画面のヘッダー・フッター・サイドバーを追加し、UI構成をオペレーターと統一
- ユーザー／オペレーターのレイアウトファイルをそれぞれ整備し、コンポーネント構造を明確化
...
- [Laravel & Vite 環境構築・テスト・デプロイまとめ](docs/laravel-vite-setup.md)
- [画面遷移図（PDF）](docs/画面遷移図.pdf)
- [ワイヤーフレーム（PDF）](docs/ワイヤーフレーム.pdf)
...
- [2025-07-31](docs/logs/2025-07-31.md)
- [2025-08-01](docs/logs/2025-08-01.md)
- [2025-07期まとめ](docs/logs/2025-07.md)
- [2025-08-08](docs/logs/2025-08-08.md)
