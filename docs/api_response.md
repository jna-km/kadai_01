...
- `status`・`message` はAPIの用途や画面要件に応じて省略可能です。
- 実装上、`data` のみ返却される場合もあります（例：`/api/user` など）。
- 必要に応じて `status` や `message` を追加してください。

### 実際のレスポンス例

- `/api/user` など一部APIでは、下記のように `data` のみ返却されます。
...
- バリデーションエラー以外（例：認証エラーや権限エラー）の場合は、`errors` フィールドが省略されることがあります。
...
| フィールド   | 必須/省略可 | 用途・備考                                 |
| ------------ | ----------- | ------------------------------------------ |
| status       | 省略可      | 成功時は省略可、失敗時は`error`で返却推奨  |
| message      | 省略可      | 概要や説明文。用途に応じて省略可           |
| data         | 必須        | メインデータ本体                           |
| errors       | 省略可      | バリデーション失敗時などに詳細を格納        |
