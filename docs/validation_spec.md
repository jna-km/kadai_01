...
| ルール名      | 説明                                 | Laravel | React |
|---------------|--------------------------------------|---------|-------|
| required      | 必須項目                             | ○       | ○     |
| email         | メールアドレス形式                   | ○       | ○     |
| unique        | DB内で一意                           | ○       | △（API連携）|
| confirmed     | パスワード確認（2回入力一致）         | ○       | ○     |
| min/max       | 文字数・数値の最小/最大               | ○       | ○     |
| regex         | 正規表現によるパターンチェック        | ○       | ○     |
| date          | 日付形式                             | ○       | ○     |
| numeric       | 数値のみ                             | ○       | ○     |
| in/not_in     | 許可値・禁止値リスト                 | ○       | ○     |
...
- **Reactフロントエンド**
  - `onChange`：入力値変更時に即時バリデーション（リアルタイムフィードバック）
